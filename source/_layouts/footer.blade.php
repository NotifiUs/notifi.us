<div class="w-full bg-purple-800 bg-footer mx-auto my-0 text-purple-100 py-8">

    <div class="flex flex-wrap py-4 max-w-6xl mx-auto">
        <div class="w-full text-center lg:text-justify my-4 flex-row lg:flex-col lg:w-1/3 mx-auto">
            <h5 class="uppercase text-2xl font-black text-purple-700">Resources</h5>
            <ul class="text-center lg:text-justify px-4">
                <li class="my-1 whitespace-no-wrap">
                    <i class="fas fa-newspaper text-purple-400"></i>
                    <a class="hover:text-purple-500" href="https://mailchi.mp/f9431b8b1b6d/call-theory">Call Theory Newsletter</a>
                </li>
                <li class="my-1 whitespace-no-wrap">
                    <i class="fas fa-shopping-cart  text-purple-400"></i>
                    <a class="hover:text-purple-500"  href="https://store.calltheory.com">Scripting & Art Store</a>
                </li>
                <li class="my-1 whitespace-no-wrap">
                    <i class="fab fa-github  text-purple-400"></i>
                    <a class="hover:text-purple-500"  href="https://github.com/NotifiUs">
                        Github Code Repository
                    </a>
                </li>
                <li class="my-1 whitespace-no-wrap">
                    <i class="fas fa-envelope  text-purple-400"></i>
                    <a class="hover:text-purple-500" href="https://wctp.io">Open-Source SMS Aggregator</a>
                </li>
                <li class="my-1 whitespace-no-wrap">
                    <i class="fas fa-life-ring  text-purple-400"></i>
                    <a class="hover:text-purple-500" href="https://calltheory.com">Support & Documentation</a>
                </li>
            </ul>
        </div>

        <div class="my-4 w-full text-center lg:text-justify flex-row lg:flex-col lg:w-1/3 mx-auto">
            <h5 class="uppercase text-2xl font-black text-purple-700">Services</h5>
            <ul class="text-center lg:text-justify px-4">
                <li class="my-1 whitespace-no-wrap">
                    <i class="fas fa-chart-pie  text-purple-400"></i>
                    <a class="hover:text-purple-500" href="https://callcenterkpi.com">Real-time KPI Dashboard</a>
                </li>
                <li class="my-1 whitespace-no-wrap">
                    <i class="fas fa-gift  text-purple-400"></i>
                    <a class="hover:text-purple-500" href="https://callcentertrunk.com">Call Center Trunk</a>
                </li>
                <li class="my-1 whitespace-no-wrap">
                    <i class="fas fa-clipboard-list  text-purple-400"></i>
                    <a class="hover:text-purple-500"  href="https://hourlycheck.com">Hourly Delivery Checks</a>
                </li>
                <li class="my-1 whitespace-no-wrap">
                    <i class="fas fa-user-ninja  text-purple-400"></i>
                    <a class="hover:text-purple-500"  href="https://tas.ninja">
                        Security Awareness
                    </a>
                </li>

                <li class="my-1 whitespace-no-wrap">
                    <i class="fas fa-check-circle  text-purple-400"></i>
                    <a class="hover:text-purple-500" href="https://callcenterscore.com">Call Center Performance Scoring</a>
                </li>
            </ul>
        </div>

        <div class="my-4 w-full text-center lg:text-justify flex-row lg:flex-col lg:w-1/3 mx-auto">
            <h5 class="uppercase text-2xl font-black text-purple-700">Follow Us</h5>
            <ul class="text-center lg:text-justify px-4">
                <li class="my-1 whitespace-no-wrap">
                    <a class="hover:text-purple-500" href="https://www.facebook.com/NotifiUsConsulting">
                        <i class="fab fa-facebook  text-purple-400"></i>
                        Facebook
                    </a>
                </li>
                <li class="my-1 whitespace-no-wrap">
                    <a class="hover:text-purple-500" href="https://www.instagram.com/notifius">
                        <i class="fab fa-instagram  text-purple-400"></i>
                        Instagram
                    </a>
                </li>
                <li class="my-1 whitespace-no-wrap">
                    <a class="hover:text-purple-500"  href="https://twitter.com/NotifiUs">
                        <i class="fab fa-twitter  text-purple-400"></i>
                        Twitter
                    </a>
                </li>
                <li class="my-1 whitespace-no-wrap">
                    <a class="hover:text-purple-500"  href="https://pinterest.com/NotifiUs">
                        <i class="fab fa-pinterest  text-purple-400"></i>
                        Pinterest
                    </a>
                </li>
                <li class="my-1 whitespace-no-wrap">
                    <a class="hover:text-purple-500"  href="https://www.linkedin.com/company/notifius">
                        <i class="fab fa-linkedin  text-purple-400"></i>
                        LinkedIn
                    </a>
                </li>
            </ul>
        </div>

    </div>

</div>

<div class="container px-4 max-w-lg mx-auto text-center my-10 clearfix">

    <p class="text-sm font-sans w-100 text-gray-500">
        <span class="text-gray-600 opacity-75">All rights reserved &copy; {{ $page->year }} <span class="text-gray-900 font-normal">NotifiUs, LLC</span></span>
    </p>

    <p class="text-sm font-sans w-100 text-gray-500 my-3">
        <a class="font-semibold hover:text-purple-500"  href="/privacy">Privacy Policy</a>
    </p>

    <p class="text-xs font-sans w-100 text-gray-700 my-8">
        View this site's source on <a title="View on Github" alt="notifius/notifi.us" class="hover:text-gray-600" href="https://github.com/notifius/notifi.us">
            <i class="fab fa-github"></i> Github
        </a>
    </p>

</div>