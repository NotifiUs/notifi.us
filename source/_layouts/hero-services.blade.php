
<div class="bg-gradient py-8 lg:py-16 w-full px-4 shadow mx-auto">
    <div class="flex flex-wrap max-w-8xl mx-auto">

        <div class="w-full lg:w-3/5 px-4 max-w-2xl mx-auto">
            <div class="container mx-auto text-center align-middle">

                <h1 class="text-white text-4xl  mb-0 mt-6">
                    @include('_layouts.universe-logo')

                </h1>

                <div class="inline-block text-2xl text-white">
                    Everything you need to level up your call center
                </div>

                <h2 class="text-white font-black text-xl uppercase font-sans my-2">
                    <span class="font-bold text-yellow-300">Premium Subscription Bundle</span>
                </h2>

                <!-- lg:flex-row || lg:flex-col w-auto lg:w-1/2 -->
                <div class="text-indigo-200 font-semibold my-3">
                    <div class="flex flex-wrap">
                        <div class="flex-row md:flex-col md:w-1/2 mx-auto">
                            Online technical documentation
                        </div>
                        <div class="flex-row md:flex-col md:w-1/2 mx-auto">
                            <i class="italic">Done With You</i> support
                        </div>

                        <div class="flex-row md:flex-col md:w-1/2 mx-auto">
                            Managed SMS aggregator
                        </div>
                        <div class="flex-row md:flex-col md:w-1/2 mx-auto">
                            Scripting library access
                        </div>

                        <div class="flex-row md:flex-col md:w-1/2 mx-auto">
                            Hourly delivery checks
                        </div>
                        <div class="flex-row md:flex-col md:w-1/2 mx-auto">
                            Technical skill evaluations
                        </div>

                        <div class="flex-row md:flex-col md:w-1/2 mx-auto">
                            Managed KPI dashboard
                        </div>
                        <div class="flex-row md:flex-col md:w-1/2 mx-auto">
                            Request learning bounties
                        </div>
                    </div>
                </div>

                <div class="my-8">
                    <a alt="Sign up at calltheory.com" class="inline rounded-lg mx-auto text-lg uppercase text-purple-100 shadow bg-purple-700 my-12 px-5 py-3 hover:bg-purple-900 hover:text-white font-bold" href="https://calltheory.com/register">
                        <i class="fas fa-check-circle text-purple-300"></i> Get started
                    </a>

                    <small class="block text-yellow-300 my-5">Level up your call center</small>
                </div>

            </div>
        </div>
        <div class="w-full lg:w-2/5 px-4">
            <div class="container mx-auto text-center lg:text-left max-w-xl">
                <div class="w-100 mx-auto">
                    <img class="w-auto mx-auto" src="/assets/images/startup.png" title="Level up with our premium subscription bundle" alt="Call Theory Universe Premium Subscription">
                </div>
            </div>
        </div>
    </div>
</div>